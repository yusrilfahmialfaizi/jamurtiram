...
class AnalisaDataTestController extends Controller
{
    ...
    function hasil_analisa(){
        return view('contents/main/hasil_analisa');
    }
    ...
    function test($train){
        ...
        ################
            ...
            // inisialisasi bobot
            // $numEpoch = 1000;
            $bX1Z1          = floatval($train["bX1Z1"]);
            $bX1Z2          = floatval($train["bX1Z2"]);
            $bX1Z3          = floatval($train["bX1Z3"]);
            $bX2Z1          = floatval($train["bX2Z1"]);
            $bX2Z2          = floatval($train["bX2Z2"]);
            $bX2Z3          = floatval($train["bX2Z3"]);
            $bX0Z1          = floatval($train["bX0Z1"]);
            $bX0Z2          = floatval($train["bX0Z2"]);
            $bX0Z3          = floatval($train["bX0Z3"]);
            $bZ0Y           = floatval($train["bZ0Y"]);
            $bZ1Y           = floatval($train["bZ1Y"]);
            $bZ2Y           = floatval($train["bZ2Y"]);
            $bZ3Y           = floatval($train["bZ3Y"]);
            ...
            #####langkah 3#####
                ...
                foreach ($dataset as $data) {
                    ...
                    $outputke = [
                        ...
                        'target'        => $data->target,
                        'output'        => $Y,
                        'output-target' => $hasil_akhir,
                        'nilai-fuzzy'   => $fuzzy['nilai_fuzzy'],
                        'hasil-fuzzy'   => $fuzzy['fuzzy_output']
                    ];
                    array_push($output, $outputke);
    ...
    function fuzzy($Y){
        ...
        return $fuzzy = ['fuzzy_output' => $fuzzy_output, 'nilai_fuzzy' => $nilai_fuzzy];
    }
}
